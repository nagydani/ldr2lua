...
function draw_line_segment(x1, y1, z1, x2, y2, z2)
...
function draw_line(_, x1, y1, z1, x2, y2, z2)
...
function draw_outline(x1, y1, z1, x2, y2, z2,
    x3, y3, z3, x4, y4, z4)
  local sx1, sy1 = screen_point(x1, y1, z1)
  local sx2, sy2 = screen_point(x2, y2, z2)
...
function draw_color_outline(_, x1, y1, z1, x2, y2, z2,
    x3, y3, z3, x4, y4, z4)
  draw_outline(x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4)
end
...
-- Draw pass: render wireframe primitives only.

local DRAW_CALLBACKS = loadtable("draw_callbacks.lua")

local function draw_ldraw(root)
  traverse_ldraw(root, DRAW_CALLBACKS, ROOT_COLOR)
end

local function draw_ref(p)
  ref_frame(p.ldraw, ROOT_COLOR, p.m, p.t)
end
...
local function draw_scene()
  ...
  if SELECTED_PART then
    ...
    draw_ldraw(function()
      draw_ref(SELECTED_PART)
    end)
  end
end

-- Mouse changes selection; love.draw renders current state.

function love.mousepressed(mx, my)
  SELECTED_PART = find_part(MODEL, mouse_ray(mx, my))
end
